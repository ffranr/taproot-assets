...
type Negotiator struct {
	...
	// offers.
	assetSellOffers map[asset.ID]SellOffer

	// assetGroupSellOffers is a map (keyed on asset group key) that holds
	// asset sell offers.
	assetGroupSellOffers map[btcec.PublicKey]SellOffer

	// assetSellOffersMtx is a mutex that protects the asset sell offers
	// maps.
	sellOffersMtx sync.RWMutex
...
func NewNegotiator(cfg NegotiatorCfg) (*Negotiator, error) {
	...
	return &Negotiator{
		...
		assetSellOffers:      make(map[asset.ID]SellOffer),
		assetGroupSellOffers: make(map[btcec.PublicKey]SellOffer),
...
func (n *Negotiator) queryAskFromPriceOracle(
	...
	if oracleResponse.AskPrice == nil {
		...
		// If the price oracle returned an error, then we will include
		// it in the reject message.
...
func (n *Negotiator) UpsertAssetSellOffer(offer SellOffer) error {
	...
	n.sellOffersMtx.Lock()
	defer n.sellOffersMtx.Unlock()
	...
	case offer.AssetGroupKey != nil:
		n.assetGroupSellOffers[*offer.AssetGroupKey] = offer

	case offer.AssetID != nil:
		n.assetSellOffers[*offer.AssetID] = offer
	}
...
func (n *Negotiator) RemoveAssetSellOffer(assetID *asset.ID,
	...
	case assetGroupKey != nil:
		delete(n.assetGroupSellOffers, *assetGroupKey)

	case assetID != nil:
		delete(n.assetSellOffers, *assetID)
...
func (n *Negotiator) HasAssetSellOffer(assetID *asset.ID,
	...
	switch {
	case assetGroupKey != nil:
		offer, ok := n.assetGroupSellOffers[*assetGroupKey]
		if !ok {
	...
	case assetID != nil:
		offer, ok := n.assetSellOffers[*assetID]
		if !ok {
			// Corresponding offer not found.
			return false
		}
